...
2. Z-score Calculation: Automatically creates Z-scores for behavioral data.
3. Structured Output: Subfolders for Raw vs Zscore analysis.
4. Zero-Size Exclusion: Subjects with Total_Size=0 are EXCLUDED from statistics (Active Only).
"""
...
# Pre-process SLRT Columns & Calculate Z-scores
print("  Processing SLRT Columns & Calculating Z-scores...")
for slrt_col in SLRT_VAR_LIST:
    if slrt_col not in pheno.columns:
        print(f"  !!! Warning: {slrt_col} not found in Excel. Skipping.")
    else:
        pheno[slrt_col] = pd.to_numeric(pheno[slrt_col], errors='coerce')
        # Calculate Z-score
        mean_val = pheno[slrt_col].mean()
        std_val = pheno[slrt_col].std()
        pheno[f"{slrt_col}_z"] = (pheno[slrt_col] - mean_val) / std_val
...
for current_metric in METRIC_LIST:
    ...
    df_final["Total_Size"] = df_final["Total_Size"].fillna(0)
    df_final["Weighted_Value"] = df_final["Weighted_Value"].fillna(0)
    ...
    for current_slrt_base in SLRT_VAR_LIST:
        ...
        for data_type in DATA_TYPE_LIST:
            ...
            # Select column
            if data_type == "Raw":
                analysis_col = current_slrt_base
            else: # Zscore
                analysis_col = f"{current_slrt_base}_z"
            ...
            # Directory: Base / Metric / SLRT_Name / Raw_or_Zscore
            ...
            for contrast in TARGET_CONTRASTS:
                ...
                for roi in TARGET_ROIS:
                    ...
                    covs_final = []
                    if "Age_c" in data.columns: covs_final.append("Age_c")
                    if "Sex_bin" in data.columns: covs_final.append("Sex_bin")
                    if "Hand_bin" in data.columns: covs_final.append("Hand_bin")
                    if "spike_ratio_FD_c" in data.columns: covs_final.append("spike_ratio_FD_c")

                    # ============================================================
                    #  FILTER: EXCLUDE ZERO SIZE (Active Only)
                    #  Applied to BOTH Intensity and Size Analysis
                    # ============================================================
                    ...
                    if data_active[analysis_col].notna().sum() > 5:
                        ...
                        if model and analysis_col in model.pvalues:
                            ...
                            results.append({
                                "Contrast": contrast, "ROI": roi, "Test": "Regress_SLRT",
                                "Metric": f"Intensity_{current_metric}", "N": N,
                                "Beta": model.params[analysis_col], "P": p_val, 
                    ...
                    if data_active["Group_Bin"].nunique() > 1:
                        ...
                        if model and "Group_Bin" in model.pvalues:
                            ...
                            results.append({
                                "Contrast": contrast, "ROI": roi, "Test": "Group_Diff",
                                "Metric": f"Intensity_{current_metric}", "N": N,
                                "Beta": model.params["Group_Bin"], "P": p_val,
                                "Sig": "*" if p_val < 0.05 else ""
                            })
            ...
            if results:
                ...
                cols = ["Contrast", "ROI", "Test", "Metric", "Beta", "P", "Sig", "N"]
                final_cols = [c for c in cols if c in df_res.columns]
                df_res[final_cols].sort_values(["Contrast", "ROI"]).to_csv(out_file, index=False)
                print(f"      [Saved] {out_file.name}")
            else:
                print("      [Info] No significant results.")
